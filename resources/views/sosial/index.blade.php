<x-layouts.public title="Sosial">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6"
         x-data="sosialApp({
             ...
             charts: {
                 edu: { labels: @js($pendidikanData->keys()->values()), values: @js($pendidikanData->values()) },
                 job: { labels: @js($pekerjaanData->keys()->values()), values: @js($pekerjaanData->values()) },
                 usia_sekolah: { labels: @js($usiaSekolahData->keys()->values()), values: @js($usiaSekolahData->values()) },
                 agama: { labels: @js($agamaData->keys()->values()), values: @js($agamaData->values()) },
                 ktp_status: { labels: @js($ktpStatusData->keys()->values()), values: @js($ktpStatusData->values()) },
                 kk_status: { labels: @js($kkStatusData->keys()->values()), values: @js($kkStatusData->values()) },
                 goldar: { labels: @js($golonganDarahData->keys()->values()), values: @js($golonganDarahData->values()) },
                 shbkel: { labels: @js($shbkelData->keys()->values()), values: @js($shbkelData->values()) },
                 kia: { labels: ['Memiliki KIA', 'Belum Memiliki KIA'], values: [@js($kiaData->get('Memiliki KIA', 0)), @js($kiaData->get('Belum Memiliki KIA', 0))] },
                 akta_lahir: { labels: ['Memiliki', 'Belum Memiliki'], values: [@js($aktaLahirData->get('Memiliki Akta Lahir', 0)), @js($aktaLahirData->get('Belum Memiliki Akta Lahir', 0))] },
                 kk_jk: { labels: @js($kepalaKeluargaJkData->keys()->values()), values: @js($kepalaKeluargaJkData->values()) },
                 akta_lahir_kelurahan: @js($aktaLahirKelurahanData),
                 kia_kelurahan: @js($kiaKelurahanData),
                 ktp_kelurahan: @js($ktpKelurahanData),
             },
         })"
